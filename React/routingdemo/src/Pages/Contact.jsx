...
function Contact() {
  ...
  // }
  return (
    <div className='flex flex-col min-h-screen  justify-center items-center ' style={{backgroundColor:theme==='light'?'white':'black', color:theme==='light'?'black':'white'}}>
      ...
      toast.error("message failed to send, please try again.");
            ...
            <p className='text-red-400'>{errors.name && touched.name && errors.name}</p> 
             <input type="email" name='email' placeholder='your email' required className='rounded-2xl pl-2  mb-3 border-2' onChange={handleChange} onBlur={handleBlur} value={values.email} />
              ...
              <textarea type='text' name="message" rows="5" placeholder='Your Message' required className='rounded-2xl pl-2  mb-3 h-auto w-full  border-2' onChange={handleChange} onBlur={handleBlur} value={values.message} ></textarea>
              <p className='text-red-400'>{errors.message && touched.message && errors.message}</p> 
              <div className='text-center'>
              <button type='submit'  className=' border-amber-200 border-r-2 rounded-2xl text-center border-2 px-4  hover:border-blue-300 '> Send </button>
              </div>
          </form>}
